...
get_mirror() {
  ...
  if [ -n "$CACHED_trueos_MIRROR" ] ; then
     VAL="$CACHED_trueos_MIRROR"
     export VAL
     return
  fi

  # Set the mirror URL
  VAL="`cat ${trueos_ETCCONF} 2>/dev/null | grep 'trueos_MIRROR: ' | sed 's|trueos_MIRROR: ||g'`"
  if [ -n "$VAL" ] ; then
  ...
  touch $mFile
  fetch -o $mFile http://getmirror.trueos.org >/dev/null 2>/dev/null
...
get_mirror_loc()
{
  ...
  case $2 in
    pkg) mirrorTag="PKG_MIRROR" 
         mirrorFile="/usr/local/share/trueos/conf/pkg-mirror"
         ;;
    pbi) mirrorTag="PBI_MIRROR" 
         mirrorFile="/usr/local/share/trueos/conf/pbi-mirror"
         ;;
    iso) mirrorTag="ISO_MIRROR" 
         mirrorFile="/usr/local/share/trueos/conf/iso-mirror"
         ;;
  update) mirrorTag="UPDATE_MIRROR" 
         mirrorFile="/usr/local/share/trueos/conf/update-mirror"
         ;;
    *) exit_err "Bad mirror type!" ;;
  esac

  # Set the mirror URL
  local VAL=`cat ${trueos_ETCCONF} 2>/dev/null | grep "^${mirrorTag}:" | sed "s|^${mirrorTag}: ||g"`
  if [ -n "$VAL" ] ; then
     echo "${VAL}${1}"
     return
  fi
...
# Function to download a file from the trueos mirrors
...
run_firstboot()
{
  ...
  if [ ! -e "/var/.trueos-firstgui" ] ; then return; fi
  ...
  case $ratio in
    13) mov="trueos9_4-3_UXGA.flv";;
    16) mov="trueos9_16-10_WUXGA.flv";;
    17) mov="trueos9_16-9_1080p.flv";;
     *) mov="trueos9_4-3_UXGA.flv";;
  esac

  # Play the video now
  # NO Movie for 10, if we end up with one, replace this
  #mplayer -fs -nomouseinput -zoom /usr/local/share/trueos/movies/$mov
